...
def main():
    ...
    icon_dir = SRC / "icons"
    pngs = {
        "icon-192.png": icon_dir / "icon-192.png",
        "icon-512.png": icon_dir / "icon-512.png",
        "apple-touch-icon.png": icon_dir / "apple-touch-icon.png",
    }
    has_pngs = all(p.exists() for p in pngs.values())
    ...
    shutil.copy2(icon_dir / "icon.svg", OUT / "icons" / "icon.svg")
    if has_pngs:
        for name, p in pngs.items():
            shutil.copy2(p, OUT / "icons" / name)

    if has_pngs:
        b192 = b64_file(pngs["icon-192.png"])
        b512 = b64_file(pngs["icon-512.png"])
        b180 = b64_file(pngs["apple-touch-icon.png"])
        icon_link = f'<link rel="icon" href="{svg_uri}">\n<link rel="apple-touch-icon" href="data:image/png;base64,{b180}">' 
        manifest_icons = (
            f'{{ src: "data:image/png;base64,{b192}", sizes: "192x192", type: "image/png" }},'
            f'{{ src: "data:image/png;base64,{b512}", sizes: "512x512", type: "image/png", purpose: "any maskable" }}'
        )
    else:
        icon_link = f'<link rel="icon" href="{svg_uri}">\n<link rel="apple-touch-icon" href="{svg_uri}">' 
        manifest_icons = f'{{ src: "{svg_uri}", sizes: "any", type: "image/svg+xml", purpose: "any" }}'
        print("Hinweis: keine PNG-Icons gefunden – nutze SVG in holen.html", file=sys.stderr)
