...
def minimal_png(size: int, color=(0x12, 0x16, 0x1C), accent=(0x5A, 0xDC, 0xA0)) -> bytes:
    """Erzeugt ein einfaches PNG (solid + Download-Pfeil-ish) ohne externe Dependencies."""
    w = h = size
    raw = bytearray()
    for y in range(h):
        raw.append(0)
        for x in range(w):
            cx, cy = w // 2, h // 2
            in_circle = (x - cx) ** 2 + (y - cy) ** 2 < (size * 0.32) ** 2
            shaft = abs(x - cx) < size * 0.04 and cy * 0.55 < y < cy * 1.15
            head = abs(x - cx) < (y - cy * 1.05) * 0.9 and cy * 1.0 < y < cy * 1.35
            if shaft or head:
                r, g, b = accent
            elif in_circle:
                r, g, b = accent[0] // 4, accent[1] // 4, accent[2] // 4
            else:
                r, g, b = color
...
def ensure_icons():
    ...
    if not svg.exists():
        svg.write_text(
            ...
            '<rect width="512" height="512" rx="112" fill="#12161c"/>'
            '<circle cx="256" cy="256" r="168" fill="none" stroke="#5adca0" stroke-width="22"/>'
            '<rect x="236" y="150" width="40" height="150" rx="8" fill="#5adca0"/>'
            '<path d="M160 286 L256 392 L352 286 Z" fill="#5adca0"/></svg>\n',
            encoding="utf-8",
        )
...
<header>
  <div class="brand"><span class="logo" aria-hidden="true"></span>
    <div><h1>Holen</h1><p>Link rein. Datei raus.</p></div>
  </div>
...
<input id="url" name="url" type="url" inputmode="url" autocomplete="off" spellcheck="false" placeholder="https://… YouTube, TikTok, Instagram, X …" required>
<div class="row">
  <label>Qualität
    <select id="quality">
...
<aside id="sheet" class="sheet" hidden>
  <div class="sheet-card">
    ...
    <label>API-Key (optional)<input id="apiKey" type="password" autocomplete="off" placeholder="nur falls die Instanz einen verlangt"></label>
    <div class="presets">
    ...
    <p class="legal">Nur öffentlich erreichbare Inhalte. Was du speicherst, liegt in deiner Verantwortung. Cobalt ist kein Piraterie-Tool.</p>
  </div>
</aside>
<script>{js}</script>
<script>
